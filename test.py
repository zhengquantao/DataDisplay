...
def find_file(file: str, name: str):
    ...
    for good in group_data:
        ...
        ax1.plot(datetime, shop_score, "red", label=good[0])
        ax1.legend(title="评分")
        ax1.grid(True)
    plt.xlabel(name)
    ...
    for good in group_data:
        ...
        datetime = good[1]['date'].fillna('0')
        ax1.plot(datetime, shop_comment, "red", label=good[0])
        ax1.legend(title="评论数")
        ax1.grid(True)
    plt.xlabel(name)
    # plt.savefig('./image/'+filename+'/'+name+'.png')
    # plt.cla()
    ...
    for good in group_data:
        ...
        datetime = good[1]['date'].fillna('0')
        ax1.plot(datetime, shop_big_ranking, "red", label=good[0])
        ax1.legend(title="大类排名")
        ax1.grid(True)
    plt.xlabel(name)
    # plt.savefig('./image/'+filename+'/'+name+'.png')
    # plt.cla()
    ...
    for good in group_data:
        ...
        datetime = good[1]['date'].fillna('0')
        ax1.plot(datetime, shop_small_ranking, "red", label=good[0])
        ax1.legend(title="小类排名")
        ax1.grid(True)
    plt.xlabel(name)
    # plt.savefig('./image/'+filename+'/'+name+'.png')
    # plt.cla()
